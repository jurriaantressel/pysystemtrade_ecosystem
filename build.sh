...
if [ -f "$TARGET_ENV_FILE" ]; then
  # Check if a local .env file exists and move it to .env.old
  if [ -f ".env" ]; then
    # Move existing .env to .env.old, overwriting if .env.old already exists
    mv -f .env .env.old
  fi
  ...
  echo "Error: $TARGET_ENV_FILE does not exist. Please build the pysystemtrade_config project."
  exit 1
fi
